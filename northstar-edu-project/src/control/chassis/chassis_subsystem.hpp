...
public:
    ...
    */
    // STEP 2 HERE
    void setVelocityFieldDrive(float forwardV, float sidewaysV, float rotationalV);
    void driveBasedOnHeading(float forwardV, float sidewaysV, float rotationalV, float heading);
    ...
    {
        for (size_t i = 0; i < motorArray.size(); i++)
        {
            motorArray[i].setDesiredOutput(0);
        }
    }
...
protected:
    ...
    std::array<Motor, static_cast<uint8_t>(MotorId::NUM_MOTORS)> motorArray;
